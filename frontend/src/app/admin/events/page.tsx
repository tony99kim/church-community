...
function downloadCSV(data: Participant[]) {
  ...
  a.href = url; a.download = `참여자_${new Date().toISOString().slice(0, 10)}.csv`; a.click();
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        {tab === 'participants' && (
          <button onClick={() => downloadCSV(filteredParticipants)} className="bg-green-600 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-green-700 transition">
            CSV 다운로드
          </button>
        )}
      </div>
      ...
      {tab === 'participants' && (
        <>
          ...
          {filteredParticipants.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-200 py-16 text-center text-gray-400">참여자가 없습니다.</div>
          ) : (
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
